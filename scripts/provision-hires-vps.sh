...
# in any way. See docs/hires-audio.md for the participant/operator guide and
# docs/hires-spike.md for the spike that led to this design
# (requirements.md §4/§9, roadmap.md Phase 6, decided 2026-07-05: hi-res audio
# ships as a starter option module, not a client-fork feature).
#
# Usage:
#   sudo scripts/provision-hires-vps.sh [--max-clients 6] [--bind-port 4464] \
#     [--sample-rate 192000] [--dry-run]
#
#   sudo scripts/provision-hires-vps.sh --add-user alice [--password <pw>] [--dry-run]
#
...
usage() {
  cat >&2 <<'USAGE'
Usage: provision-hires-vps.sh [--max-clients N] [--bind-port P] \
         [--sample-rate R] [--dry-run]
       provision-hires-vps.sh --add-user NAME [--password PW] [--dry-run]
...
# Generated by scripts/provision-hires-vps.sh. Hi-res audio is an optional
# module (see docs/hires-audio.md) — start on demand, do not enable at boot.
[Unit]
...
3. Client connect command (run by each participant, see docs/hires-audio.md
...
5. Full participant/operator guide: docs/hires-audio.md
NEXT
