...
本体と結合しない拡張オプションです。詳細な利用ガイド(参加者向け導入手順・運用ルール・トラブルシュート)は [docs/hires-audio.md](hires-audio.md) を参照してください。ここでは運用者向けの要点のみ記載します。

- **起動/停止はオンデマンド**。RAM 節約のため systemd unit は既定で enable されていません。使う直前に起動し、終わったら止めます。
...
- **参加ユーザーの追加/削除**: 追加は `scripts/provision-hires-vps.sh --add-user <name>` で credsfile (`/etc/selfmatrix-hires/credentials`) に追記します。削除は credsfile から該当行を手で消して `selfmatrix-hires-hub.service` を再起動します。
- **帯域監視**: Grafana の VPS 回線パネルで見ます。ハイレゾは 1 人あたり上下 ≈10Mbps を消費するため、逼迫時は同時参加人数を減らしてください(既定上限は 6 人)。
- **RAM への注意**: hub のオーディオスタックは接続人数に応じて増え、実測では 2 クライアント接続時で ≈400MB でした (docs/hires-spike.md)。1GB クラスの VPS では常時起動せず使うときだけ起動する運用にし、既定上限 6 人〜10 人規模を日常的に回すなら 2GB クラスへの増強を前提にしてください。
